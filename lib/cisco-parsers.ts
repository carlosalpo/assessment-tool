...
export function parseCiscoEvidence(files: EvidenceFile[]): ParsedAssessment {
  ...
  for (const file of files) {
    ...
    const hostname = extractHostname(content) ?? guessHostnameFromFilename(file.name);
    const showVersion = extractShowVersion(content);
    const inventory = extractInventory(content);
    const inventoryItems = extractInventoryItems(content, hostname, file.name);
    const role = suggestRole(showVersion.model || inventory.model || "", hostname, content);
    ...
    interfaces.push(...parseInterfacesStatus(content, deviceId, hostname));
    relations.push(...parseCdpNeighbors(content, deviceId, hostname));
    relations.push(...parseLldpNeighbors(content, deviceId, hostname));
    findings.push(...generatePreliminaryFindings(content, devicesByHost.get(hostname)!, file.name));
  }
